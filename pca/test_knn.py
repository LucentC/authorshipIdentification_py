...
t1 = data_analysis.get_features_from_database_by_doc_id(223) # 1
feature_list.extend(t1)
author_list.extend([1 for x in range(len(t1))])

t2 = data_analysis.get_features_from_database_by_doc_id(357) # 2
feature_list.extend(t2)
author_list.extend([2 for x in range(len(t2))])

t3 = data_analysis.get_features_from_database_by_doc_id(370) # 3
...
for li in neigh.predict_proba(qp):
    ...
    b += li[1]
    c += li[2]
print "Writen by Mr. 11 ", (a / len(qp)) * 100, "%"
print "Writen by Mr. 12 ", (b / len(qp)) * 100, "%"
print "Writen by Mr. 13 ", (c / len(qp)) * 100, "%"
